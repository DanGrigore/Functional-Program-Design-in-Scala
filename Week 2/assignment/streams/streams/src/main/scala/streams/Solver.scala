...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val exploredBlocks = explored ++ (newNeighbors map (_._1))
      initial.head #:: from(initial.tail ++ newNeighbors, exploredBlocks)
    }

  /**
    * The stream of all paths that begin at the starting block.
    */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from(Stream((Block(startPos, startPos), Nil)), Set())
